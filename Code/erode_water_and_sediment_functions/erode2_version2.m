function [H, h, index, target] = erode2_version2(H, h, toPrint, index, target, original)
...
WaterHvideo=VideoWriter('1Capital_H_for_frame_2.avi');
Waterhvideo=VideoWriter('1Miniscule_h_for_frame_2.avi');
...
for f = 1:1:toPrint
    ...
    for t = 1:1:1000
        ...
        [h, T] = water_flow2(0.1, dx, dy, Nx, Ny, h, H, 0.1,15,1,10^(-2) );
        ...
        H = sediment_flow2(T, dx, dy, Nx, Ny, 0.1, h, H, 2, 2);
...
%         Now we will make the movie
        ...
        for n =1:2:11
        if (t==n*10)
            save(sprintf('waterframes/frame%d', t), 'H', 'h', 't')
        end
        end     
    end

    % Save the data need to plot the graph 
    save(sprintf('frames/frame%d', index), 'H', 'h', 'index')
