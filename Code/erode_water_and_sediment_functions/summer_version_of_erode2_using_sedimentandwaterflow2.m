function [H, h, index, target] = summer_version_of_erode2_using_sedimentandwaterflow2(H, h, toPrint, index, target, original)
...
%         Now we will make the movie
        ...
        for n =1:2:11
        if (t==n*10)
            save(sprintf('debrisflow_rain_millionpixels/frame%d', t), 'H', 'h', 't')
        end
        end     
    end

    % Save the data need to plot the graph 
    save(sprintf('mudflow_Mountain2_millionpixels/frame%d', index), 'H', 'h', 'index')
