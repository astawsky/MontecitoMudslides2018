...
for f = 1:1:toPrint
    ...
    for t = 1:1:1000
        ...
        [h, T] = water_flow5(.01, dx, dy, Nx, Ny, h, H, .5,15,1,10^(-2) );
...
%         writeVideo(Waterhvideo,frame);
        ...
        if mod(t,10)==0
            save(sprintf('OraSmoothingWaterFrames/frame_%d_%d', index, t), 'H', 'h', 't')
        end  
    end

    % Save the data need to plot the graph 
    save(sprintf('OraMudFrames/frame_%d', index), 'H', 'h', 'index')
