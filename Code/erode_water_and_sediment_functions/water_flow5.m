function [h, T] = water_flow5(dt, dx, dy, Nx, Ny, h, H, R, eta, transport, tol)
    ...
    for j = 2:(Ny-1)
        for i = 1:(Nx) 
            ...
            if i>1 
                if i<Nx
                    if j>1
            h(i, j) = hn(i, j) + (dt/(eta^2))*(R - (fluxPlusX - fluxMinusX)/dx - (fluxPlusY - fluxMinusY)/dy)+(10^3)*(dt/(eta^2))*(h(i+1,j)+h(i-1,j)-2*h(i,j)+h(i,j+1)+h(i,j-1)-2*h(i,j));
            else
