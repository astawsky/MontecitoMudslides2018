function [H, h, index, target] = summer_version_of_erode2_using_sedimentandwaterflow2_version2(H, h, toPrint, index, target, original)
...
for f = 1:1:toPrint
    ...
    for t = 1:1:1000
        ...
        [h, T] = water_flow5(.001, dx, dy, Nx, Ny, h, H, .5,15,1,10^(-3) );
...
%         writeVideo(Waterhvideo,frame);
        ...
        if mod(t,1)==0
            save(sprintf('DifferentsmallhWaterFrames/frame_%d_%d', index, t), 'H', 'h', 't')
        end  
    end

    % Save the data need to plot the graph 
    %save(sprintf('ReducedDEMSmoothingMudframes/frame_%d', index), 'H', 'h', 'index')
