function [h, T] = water_flow4(dt, dx, dy, Nx, Ny, h, H, R, eta, transport, tol)
...
while (~equilibrium )
    ...
    for j = 2:(Ny-1)
        for i = 1:(Nx) 
            ...
            if i>1 
                if i<Nx
            h(i, j) = hn(i, j) + (dt/(eta^2))*(R - (fluxPlusX - fluxMinusX)/dx - (fluxPlusY - fluxMinusY)/dy)+(10^3)*(dt/(eta^2))*(h(i+1,j)+h(i-1,j)-2*h(i,j));
            else
