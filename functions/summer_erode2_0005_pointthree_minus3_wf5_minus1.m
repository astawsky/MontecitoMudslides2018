function [H, h, index, target] = summer_erode2_0005_pointthree_minus3_wf5_minus1(H, h, toPrint, index, target, original)
...
for f = 1:1:toPrint
    ...
    for t = 1:1:1000
        ...
        [h, T] = water_flow5_power1(.0001, dx, dy, Nx, Ny, h, H, .5,15,1,10^(-3.5) );
...
%         writeVideo(Waterhvideo,frame);
        ...
        if mod(t,5)==0 && index<6
            save(sprintf('/home/astawsky/FoldersForKnot/11KWaterFrames_0005_pointthree_minus3_wf5_minus1/frame_%d_%d', index, t), 'H', 'h', 't')
        end  
    end

    % Save the data need to plot the graph 
    save(sprintf('/home/astawsky/FoldersForKnot/11KMudFrames_0005_pointthree_minus3_wf5_minus1/frame_%d', index), 'H', 'h', 'index')
